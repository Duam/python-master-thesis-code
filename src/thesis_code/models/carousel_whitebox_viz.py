...
"""
  Author: Paul Daum
  Date: 2019/06/25
  Brief: This script supplies methods for plotting and analysis
         of trajectories of the carousel whitebox model
"""

import casadi as cas
from casadi import DM, horzcat, blockcat
import numpy as np
from thesis_code.models.carousel_whitebox.carousel_whitebox import CarouselWhiteBoxModel
import matplotlib.pyplot as plt

""" =========================================       SIMPLE PLOTS       ============================================= """
def plotStates(model:CarouselWhiteBoxModel, dt:float, 
               Xs_ref:DM=None, Us_ref:DM=None,
               Xs_sim:DM=None, Us_sim:DM=None, 
               Xs_est:DM=None):
  """ Plots the states and input as a timeseries
  Args:
    model[CarouselWhiteBoxModel] -- The used carousel model instance
    dt[float] -- Timestep
    Xs_ref[casadi.DM] -- State trajectory reference (default None)
    Us_ref[casadi.DM] -- Control trajectory reference (default None)
    Xs_sim[casadi.DM] -- State trajectory actual (default None)
    Us_sim[casadi.DM] -- Control trajectory actual (default None)
    Xs_est[casadi.DM] -- State trajectory estimate (default None)
  """
  NX = model.NX()
...
def plotStates_withRef(Xs:np.ndarray, Xs_ref:np.ndarray, 
                       ...
                       dt:float, model:CarouselWhiteBoxModel):
  """ Plots the states and input as a timeseries
  Args:
...
def plotAerodynamics(Xs:np.ndarray, Us:np.ndarray, dt:float, model:CarouselWhiteBoxModel):
...
def plotMoments(Xs:np.ndarray, Zs:np.ndarray, Us:np.ndarray, dt:float, model:CarouselWhiteBoxModel):
  ...
  for k in range(3):
    plt.sca(ax[k])
    plt.title(r"$"+plot_string[k]+"$-components")
...
def plotFlightTrajectory(Xs:np.ndarray, model:CarouselWhiteBoxModel, roll_ref:np.ndarray = None):
