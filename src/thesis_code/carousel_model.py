...
class CarouselWhiteBoxModel:
  ...
  def get_VectorDefinedIn_X_ViewedFrom_Y( self, X:int, Y:int, vector:SX, psi:SX, phi:SX, theta:SX ):
    ...
    """
    assert 1 <= X <= 4 and 1 <= Y <= 4, "Source/Dest must be one of the four frames."
    source = X
    dest = Y
  ...
  def get_PositionDefinedIn_X_ViewedFrom_Y(self, X:int, Y:int, pos:SX, psi:SX, phi:SX, theta:SX):
    ...
    transform = lambda start,end,v: self.get_VectorDefinedIn_X_ViewedFrom_Y(start,end,v,psi,phi,theta)
  ...
  def __init__(self, params=None, with_angle_output=False, with_imu_output=True):
    ...
    self.rotateView    = lambda x,src,dest,vec: self.get_VectorDefinedIn_X_ViewedFrom_Y  (src,dest,vec,x[2],x[0],x[1])
    self.transformView = lambda x,src,dest,pos: self.get_PositionDefinedIn_X_ViewedFrom_Y(src,dest,pos,x[2],x[0],x[1])
